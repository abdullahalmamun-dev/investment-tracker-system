...
const calculateTotalProfit = async () => {
  try {
    ...
    const totalProfit = investments.reduce((sum, inv) => sum + inv.profitOrLossAmount, 0);
    return totalProfit;
  } catch (error) {
...
const calculateTotalLoss = async () => {
  try {
    ...
    const totalLoss = investments.reduce((sum, inv) => sum + Math.abs(inv.profitOrLossAmount), 0);
    return totalLoss;
  } catch (error) {
...
const sendTransactionEmail = (transactionDetails) => {
  ...
  const mailOptions = {
    ...
    to: 'mdabdullah.allinfozone@gmail.com', // Replace with your recipient email
          ...
          <div style="background-color: #ffffff; padding: 30px; border-radius: 8px; max-width: 600px; margin: 0 auto;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
              <tr>
                ...
                <td style="padding: 10px; background-color: #f9f9f9; color: #333;">${transactionDetails.name}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Profit/Loss</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: ${transactionDetails.profitOrLoss === 'profit' ? 'green' : 'red'};">${transactionDetails.profitOrLoss.charAt(0).toUpperCase() + transactionDetails.profitOrLoss.slice(1)}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Amount</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: #333;">$${transactionDetails.amount}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Description</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: #333;">${transactionDetails.description}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Profit/Loss Amount</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: ${transactionDetails.profitOrLoss === 'profit' ? 'green' : 'red'};">$${transactionDetails.profitOrLossAmount}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Date</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: #333;">${transactionDetails.date}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Balance</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: #333;">$${transactionDetails.balance}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Total Profit</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: green;">$${transactionDetails.totalProfit}</td>
              </tr>
              <tr>
                <td style="padding: 10px; background-color: #f9f9f9; font-weight: bold; color: #333;">Total Loss</td>
                <td style="padding: 10px; background-color: #f9f9f9; color: red;">$${transactionDetails.totalLoss}</td>
              </tr>
            </table>
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      console.log("Error sending email: ", error); // This will show any error
    } else {
      console.log('Email sent: ' + info.response); // Check if the email was sent
    }
  });
};
...
router.post("/", async (req, res) => {
  try {
    ...
    await investment.save(); // Save the investment
    console.log("Investment saved:", investment);

    // Calculate balance and profit/loss data
    ...
    res.status(201).json(investment); // Send the response
  } catch (error) {
...
router.get("/api/investments/:id/pdf", async (req, res) => {
  ...
  doc.font("Helvetica-Bold").fontSize(18).fillColor("rgb(0, 51, 102)") // Dark Blue Color
  ...
  // Investment Data
  doc.font("Helvetica-Bold").fontSize(14).fillColor("rgb(0, 51, 102)") // Heading color
  ...
  doc.font("Helvetica-Bold").fontSize(14).fillColor("rgb(0, 51, 102)")
    .text("Profit/Loss: ", { continued: true });

  doc.font("Helvetica").fontSize(14).fillColor(investment.profitOrLoss === "profit" ? "green" : "red")
    .text(investment.profitOrLoss.charAt(0).toUpperCase() + investment.profitOrLoss.slice(1)); // Capitalize
  ...
  doc.moveDown();
  doc.font("Helvetica-Italic").fontSize(10).fillColor("rgb(0, 51, 102)")
    .text("Investment Tracker - Generated on " + new Date().toLocaleString(), { align: "center" });
